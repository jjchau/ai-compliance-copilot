...
### Compliance classification accuracy is necessary but insufficient for product success

Even with strong compliance classification, workflow routing is what determines whether the system recommends the right operational response. Good routing has to account for more than the compliance label, including missing information, severity differences, urgency, reviewer authority, organizational risk tolerance, and limited review capacity.

### Better retrieval does not automatically produce a better product outcome

Retrieval benchmarking showed that retrieval quality could improve without materially improving end-to-end workflow routing. This suggested that product performance tuning should take a broader view across evidence representation, scoring logic, routing thresholds, and workflow-label alignment.
...
The system behaved conservatively, which is appropriate for compliance review, but conservative routing also created unnecessary escalation of low-risk cases. Avoiding false negatives is necessary, but a useful compliance copilot must also preserve reviewer capacity.
...
The trust proxy translated a broad trust concern into specific behaviours that could be observed or measured, such as correct assessments, confidently incorrect outputs, unnecessary escalation, and recoverable errors. The lesson is that trust should be broken down into the system behaviours that create or damage appropriate reliance before it is treated as a validated product outcome.
...
### 15.1 Clone and Set Up the Project
...
### 15.6 Run the Frontend Dashboard Prototype (Optional)
...
### 15.7 Run the Evaluation Notebook
```powershell
PS C:\install-directory-path> Set-Location .\ai-compliance-copilot
PS C:\install-directory-path\ai-compliance-copilot> .\.venv\Scripts\Activate.ps1
...
### 15.8 Run the Retrieval Benchmark Notebook (Optional)
```powershell
PS C:\install-directory-path> Set-Location .\ai-compliance-copilot
PS C:\install-directory-path\ai-compliance-copilot> .\.venv\Scripts\Activate.ps1
