  ...
  return (
    <div className="flex flex-col h-full justify-between min-h-0 space-y-3 w-full table-fixed">
      ...
      <div className="grid grid-cols-3 gap-3 flex-1 min-h-0 py-1">
        ...
        <div className="bg-slate-950/30 border border-slate-800/60 p-3 rounded-lg flex flex-col justify-between min-w-0">
          ...
          <div className="text-xs grid grid-cols-2 gap-x-3 gap-y-3 flex-1 items-center text-slate-300 min-w-0">
            ...
            <div className="min-w-0"><span className="text-slate-500 text-[9px] uppercase font-bold tracking-wider block mb-0.5">Notional</span><div className="truncate font-mono font-medium text-slate-400">${formatCurrency(c.notional_value)}</div></div>
            <div className="min-w-0"><span className="text-slate-500 text-[9px] uppercase font-bold tracking-wider block mb-0.5">Timestamp</span><div className="truncate font-mono text-[10px] text-slate-400">{c.timestamp || "—"}</div></div>
          </div>
        </div>
        ...
        <div className="bg-slate-950/30 border border-slate-800/60 p-3 rounded-lg flex flex-col justify-between min-w-0">
          ...
          <div className="text-xs grid grid-cols-2 gap-x-3 gap-y-3 flex-1 items-center text-slate-300 min-w-0">
            ...
            <div className="min-w-0"><span className="text-slate-500 text-[9px] uppercase font-bold tracking-wider block mb-0.5">Risk Profile</span><div className="truncate font-black text-amber-400">{c.risk_tolerance || "—"}</div></div>
            <div className="min-w-0"><span className="text-slate-500 text-[9px] uppercase font-bold tracking-wider block mb-0.5">Experience</span><div className="truncate text-slate-400">{c.investment_experience || "—"}</div></div>
        ...
        <div className="bg-slate-950/30 border border-slate-800/60 p-3 rounded-lg flex flex-col justify-between min-w-0">
          ...
          <div className="text-xs grid grid-cols-2 gap-x-3 gap-y-3 flex-1 items-center text-slate-300 min-w-0">
            ...
            <div className="min-w-0"><span className="text-slate-500 text-[9px] uppercase font-bold tracking-wider block mb-0.5">Exp / Risk History</span><div className="truncate text-slate-400">{c.advisor_experience || "—"} / <span className="text-rose-400 font-mono font-bold">{c.advisor_history_risk ?? 0}</span></div></div>
            ...
            <div className="min-w-0"><span className="text-slate-500 text-[9px] uppercase font-bold tracking-wider block mb-0.5">Desk Context</span><div className="truncate text-slate-400 text-[11px] italic font-serif leading-tight">{c.advisor_notes || "None"}</div></div>
          </div>
        </div>
      </div>

      {/* Tweaked Policy & Exception Dashboard Row */}
      <div className="border-t border-slate-800/80 pt-2 shrink-0 bg-slate-950/40 p-2 rounded-lg grid grid-cols-12 gap-4 items-start w-full overflow-hidden">
        ...
        {/* Exception Field (Left: 7 columns wide with internal vertical scrolling bounds) */}
        <div className="col-span-7 flex gap-2 min-w-0 max-h-[52px]">
          ...
          <div className="flex flex-col min-w-0 overflow-hidden">
            <span className="text-[9px] uppercase text-slate-500 font-black tracking-wider block mb-0.5 shrink-0">Exception Rule</span>
            <div className="text-[11px] font-semibold text-slate-200 overflow-y-auto pr-1 leading-relaxed max-h-[38px] break-words whitespace-normal custom-scrollbar">
              {isAutoPassed 
                ? "Trade met all baseline validation parameters. System automatically designated this event as straight-through cleared without active human intervention flags." 
        ...
        {/* Regulations List (Right: 5 columns wide with full elastic wrap wrapping) */}
        <div className="col-span-5 flex flex-col min-w-0 max-h-[52px]">
          ...
          <div className="flex flex-wrap gap-1 items-start content-start overflow-y-auto max-h-[38px] pr-1 custom-scrollbar w-full">
      ...
      <ExecutionFormTray
        ...
        isAutoPassed={false} // Always keep operational controls interactive
