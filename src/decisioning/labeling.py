...
def assign_case_type(trade: Trade) -> Literal["Suitability Violation", "KYC Missing", "Insufficient Experience", "Risk Signal", "Aligned Recommendation"]:
    ...
    elif any([is_horizon_mismatch(trade), is_objective_mismatch(trade), is_overexposure(trade), is_kyc_uncertain(trade) ]):
        return "Risk Signal"
    else:
...
def assign_difficulty(trade: Trade) -> Literal['Easy', 'Medium', 'Hard']:
    ...
    soft_signals = sum([
        is_horizon_mismatch(trade),
        is_objective_mismatch(trade),
